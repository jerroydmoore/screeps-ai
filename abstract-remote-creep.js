...
class AbstractRemoteCreep extends CreepsBase {
    ...
    findFreeFlag(roomExclusions=[], ignoreTtl=false) {
        ...
        for (let flagName in Game.flags) {
            ...
            if(this.creepIdFieldName && flag.memory[this.creepIdFieldName] && Game.getObjectById(flag.memory[this.creepIdFieldName])) {
            ...
            if (!ignoreTtl && flag.memory[this.ttlFieldName] && flag.memory[this.ttlFieldName] > Game.time) {
                ...
                console.log(this.roleName + ' disqualified. TTL. ' + `${flag.memory[this.ttlFieldName]} > ${Game.time} = ${flag.memory[this.ttlFieldName] > Game.time}`);
                continue;
            }
